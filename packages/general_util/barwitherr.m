...
%
%   07/02/2015  Daniel L. Kimmel        Updated for 2015b graphics
%
%**************************************************************************
...
if nRows > 1
    ...
    for col = 1:nCols
        ...
        elseif verLessThan('matlab', '8.5')
            ...
            %%% ADDED BY DANIEL KIMMEL, 7 FEB 2016, ON BASIS OF:
        ...
        % DK added -- make error bar color same as axes color
        set(hErrorbar(col), 'Color', get(gca,'XColor'));
    ...
    hErrorbar = errorbar(mean(x,1), values, lowerErrors, upperErrors, '.');
    set(hErrorbar, 'marker', 'none');
    % DK added -- make error bar color same as axes color
    set(hErrorbar, 'Color', get(gca,'XColor'));
end
